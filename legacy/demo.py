...
class AdvancedSleepStageClassifier:
    ...
    def _train_combined_model(self):
        ...
        # Store trained pipeline for day quality prediction
        self.trained_pipeline_a = pipeline
        self.label_encoder_a = label_encoder
    ...
    def predict_day_quality(self, sample_data=None, use_random_sample=True):
        """
        Predict what type of day it was based on sleep patterns
        """
        print("\n" + "=" * 60)
        print("🌟 DAY QUALITY PREDICTION BASED ON SLEEP PATTERNS")
        print("=" * 60)

        # Use the best trained model (Option A by default)
        if hasattr(self, 'trained_pipeline_a'):
            pipeline = self.trained_pipeline_a
            label_encoder = self.label_encoder_a
            data_source = self.df_combined
        else:
            print("❌ No trained model available. Please run the classification first.")
            return None

        # Get sample data for prediction
        if sample_data is None and use_random_sample:
            # Use a random sample from the dataset
            sample_idx = np.random.randint(0, len(data_source))
            sample_features = data_source.iloc[sample_idx][self.feature_cols].values.reshape(1, -1)
            actual_label = data_source.iloc[sample_idx]['label']
            print(f"📊 Analyzing random sample #{sample_idx}")
            print(f"   Actual sleep stage: {actual_label}")
        elif sample_data is not None:
            sample_features = np.array(sample_data).reshape(1, -1)
            actual_label = "Unknown"
            print("📊 Analyzing provided sample data")
        else:
        ...
        # Make prediction
        try:
            predicted_stage_encoded = pipeline.predict(sample_features)[0]
            predicted_stage = label_encoder.inverse_transform([predicted_stage_encoded])[0]
            prediction_proba = pipeline.predict_proba(sample_features)[0]
        ...
        # Calculate sleep quality metrics from the sample
        features_dict = {}
        for i, col in enumerate(self.feature_cols):
            features_dict[col] = sample_features[0][i]

        # Analyze day quality based on sleep patterns
        day_quality = self._analyze_day_quality(predicted_stage, features_dict, prediction_proba)
        
        return {
            'predicted_stage': predicted_stage,
            'actual_stage': actual_label,
            'day_quality': day_quality,
            'prediction_confidence': dict(zip(class_names, prediction_proba)),
            'features': features_dict
        }

    def _analyze_day_quality(self, predicted_stage, features, prediction_proba):
        """
        Analyze day quality based on sleep stage prediction and features
        """
        print(f"\n🔍 ANALYZING DAY QUALITY...")
        ...
        # Stage-based scoring
        stage_scores = {
            'Wake': -2,  # Too much wake time indicates poor sleep
            'Sleep stage 1': -1,  # Light sleep, not very restorative
            'Sleep stage 2': 1,   # Normal sleep stage
            'Deep Sleep (N3)': 3, # Most restorative sleep
            'Sleep stage REM': 2  # Important for mental health
        }
        
        sleep_quality_score += stage_scores.get(predicted_stage, 0)
        
        # Confidence-based adjustment
        max_confidence = max(prediction_proba)
        if max_confidence > 0.8:
            confidence_bonus = 1
            positive_indicators.append(f"High prediction confidence ({max_confidence:.2f})")
        elif max_confidence < 0.5:
            confidence_bonus = -1
            stress_indicators.append(f"Low prediction confidence ({max_confidence:.2f}) - irregular patterns")
        else:
            confidence_bonus = 0
            
        sleep_quality_score += confidence_bonus
        
        # Feature-based analysis (assuming common EEG/sleep features)
        feature_names = list(features.keys())
        
        # Look for patterns in feature values that might indicate stress/quality
        high_frequency_features = [f for f in feature_names if any(x in f.lower() for x in ['beta', 'gamma', 'high', 'freq'])]
        low_frequency_features = [f for f in feature_names if any(x in f.lower() for x in ['delta', 'theta', 'alpha', 'low'])]
        
        # High frequency activity might indicate stress/alertness during sleep
        if high_frequency_features:
            high_freq_values = [features[f] for f in high_frequency_features]
            avg_high_freq = np.mean(high_freq_values)
            if avg_high_freq > np.percentile(list(features.values()), 75):
                sleep_quality_score -= 1
                stress_indicators.append("High frequency brain activity detected (potential stress)")
        
        # Low frequency activity is generally associated with deeper, more restful sleep
        if low_frequency_features:
            low_freq_values = [features[f] for f in low_frequency_features]
            avg_low_freq = np.mean(low_freq_values)
            if avg_low_freq > np.percentile(list(features.values()), 60):
                sleep_quality_score += 1
                positive_indicators.append("Good low frequency brain activity (restful sleep)")
                
        # Additional stage-specific indicators
        if predicted_stage == 'Deep Sleep (N3)':
            positive_indicators.append("Deep sleep stage - excellent for physical recovery")
        elif predicted_stage == 'Sleep stage REM':
            positive_indicators.append("REM sleep - important for memory and emotional processing")
        elif predicted_stage == 'Wake':
            stress_indicators.append("Wake stage during sleep period - possible sleep disruption")
        elif predicted_stage == 'Sleep stage 1':
            stress_indicators.append("Light sleep stage - may indicate restless night")

        # Determine day quality based on scoring
        if sleep_quality_score >= 3:
            day_type = "Good Day"
            day_description = "Well-rested with good sleep quality"
            emoji = "😊"
        elif sleep_quality_score <= -1:
            day_type = "Bad Day"
            day_description = "Poor sleep quality, likely stressed and tired"
            emoji = "😴"
        else:
            day_type = "Moderate Day"
            day_description = "Average sleep quality, manageable energy levels"
            emoji = "😐"

        print(f"\n{emoji} PREDICTED DAY QUALITY: {day_type}")
        print(f"   Description: {day_description}")
        print(f"   Sleep Quality Score: {sleep_quality_score}")
        ...
        if stress_indicators:
            print(f"\n⚠️  Stress/Fatigue Indicators:")
            for indicator in stress_indicators:
                print(f"   • {indicator}")

        # Recommendations based on day quality
        print(f"\n💡 RECOMMENDATIONS:")
        if day_type == "Good Day":
            print("   • Great sleep! You should feel energized and focused today")
            print("   • Good day for challenging tasks and important decisions")
            print("   • Maintain your current sleep routine")
        elif day_type == "Bad Day":
            print("   • Take it easy today - avoid stressful situations if possible")
            print("   • Consider light exercise and relaxation techniques")
            print("   • Focus on getting better sleep tonight")
            print("   • Stay hydrated and avoid caffeine late in the day")
        else:  # Moderate Day
            print("   • You should have decent energy levels today")
            print("   • Good for routine tasks, but maybe avoid major decisions")
            print("   • Consider what might improve your sleep quality")
    ...
    def run_multiple_day_predictions(self, num_samples=5):
        """
        Run day quality predictions on multiple random samples
        """
        print("\n" + "=" * 60)
        print(f"🔮 RUNNING {num_samples} DAY QUALITY PREDICTIONS")
        print("=" * 60)
        
        predictions = []
        day_quality_summary = {'Good Day': 0, 'Moderate Day': 0, 'Bad Day': 0}
        
        for i in range(num_samples):
            print(f"\n--- Sample {i+1} ---")
            result = self.predict_day_quality()
            if result:
                predictions.append(result)
                day_quality_summary[result['day_quality']['day_type']] += 1
        
        # Summary
        print(f"\n" + "=" * 60)
        print("📈 DAY QUALITY SUMMARY")
        print("=" * 60)
        total = sum(day_quality_summary.values())
        for day_type, count in day_quality_summary.items():
            percentage = (count / total) * 100 if total > 0 else 0
            print(f"  {day_type}: {count}/{total} ({percentage:.1f}%)")
            
        return predictions, day_quality_summary
...
def main():
    ...
    """
    print("ADVANCED SLEEP STAGE CLASSIFICATION WITH DAY QUALITY PREDICTION")
    ...
    try:
        ...
        # Try both approaches
        classifier.option_a_combine_deep_sleep()
        ...
        # Now run day quality predictions - with error handling
        try:
            print("\n🌟 STARTING DAY QUALITY PREDICTION MODULE...")
            
            # Single prediction
            day_prediction = classifier.predict_day_quality()
            
            # Multiple predictions for variety
            predictions, summary = classifier.run_multiple_day_predictions(num_samples=3)
            
            print(f"\n" + "=" * 70)
            print("COMPLETE ANALYSIS FINISHED!")
            print("Sleep Stage Classification + Day Quality Prediction")
            print("=" * 70)
            
            return classifier, results, recommendation, predictions
            
        except Exception as day_pred_error:
            print(f"\n⚠️ Day quality prediction failed: {day_pred_error}")
            print("Returning classification results only...")
            return classifier, results, recommendation, None
